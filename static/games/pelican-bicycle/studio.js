(() => {
  'use strict';
  const M=CoastMotion, W=CoastWorld, A=CoastAnatomy, $=id=>document.getElementById(id), NS='http://www.w3.org/2000/svg';
  const reduced=matchMedia('(prefers-reduced-motion: reduce)');
  const initialState=()=>({...M.defaults,elements:W.defaultElements(),playing:!reduced.matches,reduceMotion:reduced.matches});
  let state=initialState();
  const world=W.create(document);
  const clock={motion:0,weather:0,hour:10.5};
  let frame=0,previous=0,lastMinute=-1,bellUntil=0,audio;
  function getAudio(){const Audio=window.AudioContext||window.webkitAudioContext;if(!Audio)throw new Error('当前浏览器不支持音频');return audio||=new Audio();}
  const music=CoastMusic.create(getAudio,()=>state);
  ...
  $('character-symbols').innerHTML=Object.entries(A.bodies).map(([key,body])=>`<path id="torso-${key}" d="${body.shape}"/>`).join('')+
    Object.entries(A.tails).map(([key,tail])=>`<g id="tail-${key}">${tail}</g>`).join('')+`
    ...
    <g id="head-dog"><path d="M-2-14V13H27V-11" fill="#c7a176"/><path d="M-25-36Q-30-74 5-80Q44-82 45-47L68-37Q82-26 66-15L29 1Q-15 9-25-36Z" fill="#c7a176"/><path d="M-20-67Q-53-67-40-18Q-21 11-9-30L-7-69Z" fill="#8e7356"/><path d="M45-35Q66-48 73-29Q73-12 47-14" fill="#e4ceb0"/><ellipse cx="71" cy="-33" rx="9" ry="6" fill="#425b50"/><path d="M48-13q-2 14 9 12q11-1 8-15" fill="#cb9585" stroke-width="2"/>${eye('dog',28,-41,4)}</g>
    <g id="head-panda"><path d="M-4-15V13H30V-10" fill="#f1efdd"/><circle cx="-18" cy="-67" r="19" fill="#50655c"/><circle cx="43" cy="-66" r="18" fill="#50655c"/><path d="M-28-34Q-33-79 7-82Q49-86 51-42Q67-10 32 3Q-18 17-28-34Z" fill="#f1efdd"/><ellipse cx="25" cy="-42" rx="13" ry="17" transform="rotate(23 25 -42)" fill="#50655c"/><ellipse cx="38" cy="-19" rx="22" ry="15" fill="#ebe7d0"/><ellipse cx="48" cy="-25" rx="8" ry="5" fill="#42564c"/><path d="M47-20v8l-10 2" fill="none" stroke-width="2"/><circle cx="27" cy="-43" r="6" fill="#ede9d2" stroke="none"/>${eye('panda',27,-43,3.5)}</g>
    <g id="head-frog"><path d="M-6-15V13H30V-10" fill="#97b57c"/><ellipse cx="10" cy="-30" rx="43" ry="34" fill="#97b57c"/><circle cx="-11" cy="-62" r="19" fill="#97b57c"/><circle cx="27" cy="-61" r="21" fill="#97b57c"/><circle cx="-8" cy="-62" r="10" fill="#e8e6bb" stroke="none"/><circle cx="29" cy="-60" r="12" fill="#e8e6bb" stroke="none"/><circle cx="-5" cy="-62" r="3" fill="#294e46" stroke="none"/>${eye('frog',27,-58,4)}<path d="M-10-18Q25-4 47-23" fill="none" stroke-width="2.5"/><circle cx="-13" cy="-29" r="7" fill="#cbb389" stroke="none"/></g>
    <g id="head-penguin"><path d="M-6-18V13H29V-17" fill="#506974"/><path d="M-25-34Q-30-87 11-88Q52-89 51-45Q59-8 26 2Q-19 7-25-34Z" fill="#506974"/><path d="M5-68Q35-82 43-48Q59-14 26-6Q-1-7-6-36Z" fill="#eee9d4" stroke="none"/><path d="M40-38 82-24 42-16Z" fill="#dca854" stroke-width="2.5"/>${eye('penguin',30,-48,4.2)}</g>
    ...
    <g id="hair-braid" fill="#69523e"><path d="M-31 10Q-37-27 4-24Q36-24 39 10L15 4-2 10-15 3Z"/><path d="M-27 9q-27 12-8 28q-19 13-4 27q-19 11-6 26" fill="none" stroke="#69523e" stroke-width="16"/><path d="M-35 34l9 7m-12 19 8 7m-16 19 11 3" stroke="#c08c6b" stroke-width="4"/></g>
    <g id="hair-mohawk"><path d="M-23 11-34-22-16-14-12-39 13-22 28-34 38 5 18 2Z" fill="#ad7182"/><path d="M-9-8 0-20 16-9" fill="none" stroke="#d5a3ac" stroke-width="4"/></g>
    <g id="foot-dog" fill="currentColor"><path d="M-22-19H0L2-8Q29-10 28 6H-25Z"/><path d="M10-2v8m9-8v8" stroke-width="2"/></g>
    <g id="foot-panda" fill="#50655c"><path d="M-25-20H1L3-9Q31-10 29 6Q0 13-29 4Z"/><path d="M8 0v6m9-7v7m8-7v6" stroke="#e9e4cc" stroke-width="3"/></g>
    <g id="foot-frog" fill="currentColor"><path d="M-18-19H-1L5-9 40-12 19-1 47 8 16 5 5 17-3 6-30 12-20-5Z"/><path d="M-6-5 19-1M-6-5 16 5M-6-5-3 6" fill="none" stroke="#64875e" stroke-width="1.8"/><g fill="#bdd099" stroke="none"><circle cx="39" cy="-12" r="3"/><circle cx="45" cy="8" r="3"/><circle cx="5" cy="16" r="3"/></g></g>
    <g id="foot-penguin" fill="#d9a859"><path d="M-17-17H-1L28 0 12 7 5 3-2 8-22 1Z"/><path d="M1-7 12 5M-5-5-2 5" stroke-width="1.5"/></g>
    ...
    <g id="foot-rabbit" fill="currentColor"><path d="M-23-20Q-8-26 0-12L6-7Q50-17 55-1Q52 9 24 8L-25 4Z"/><path d="M35-3v10m9-11v9" stroke-width="1.5"/><path d="M2 0Q20-6 34-3" stroke="#f7f0df" stroke-width="4" fill="none"/></g>
    ...
    <g id="shoe-hiking"><path d="M-24-29H0L4-10 28-3 30 8H-28Z" fill="#ac885d"/><path d="M-26 6H30" stroke="#445e52" stroke-width="8"/><path d="M-7-22H1m-8 8h12M0-7l11 2" stroke="#eee1b6" stroke-width="3"/><path d="M17-4V5" stroke="#6d7554" stroke-width="4"/></g>
    <g id="shoe-slippers"><path d="M-23-12H-4Q1-34 21-21Q43-9 32 8H-27Z" fill="#d8b1a9"/><path d="M2-20 1-36 13-25 22-37 28-18" fill="#d8b1a9"/><circle cx="13" cy="-12" r="2"/><circle cx="25" cy="-12" r="2"/><path d="M-24 8H32" stroke="#eee3cc" stroke-width="5"/></g>
  ...
  const parts={};
  ['tail-rig','head-rig','body-tail','body-shape','body-markings','clothes-layer','clothes-shape','clothes-details','clothes-boundary','head-shape','hair-rig','hair-shape','scarf','scarf-tail','hat','glasses','backpack','torso-rig','bicycle','boat','boat-seat','paddle','splash','clouds','overcast','stars','sun','moon','sky-top','sky-bottom','water-top','water-bottom','night-tint','fog','rear-spokes','front-spokes','crank','bell-rings'].forEach(id=>parts[id]=$(id));
  const limbs=['far','near'].map(side=>({
    leg:{root:$(side+'-leg'),upper:$(side+'-leg').querySelector('.leg-upper'),lower:$(side+'-leg').querySelector('.leg-lower'),ankle:$(side+'-leg').querySelector('.leg-ankle'),detail:$(side+'-leg').querySelector('.leg-detail'),foot:$(side+'-leg').querySelector('.foot'),use:$(side+'-leg').querySelector('use'),pedal:$(side+'-leg').querySelector('.pedal')},
  ...
  function buildWorld(){
    ...
    for(const id of ['boat','boat-seat','paddle','splash'])show(id,rowing);
    ...
    world.build(state);
    document.querySelector('.scene-label').textContent='THE SCENIC ROUTE · '+W.scenes[state.scene].route;
    $('surface-note').textContent='划艇目的地：'+W.scenes[state.scene].surface;
    const choices=$('background-elements');choices.replaceChildren();
    for(const [depth,layer] of Object.entries(W.layers)){
      const label=document.createElement('label'),select=document.createElement('select');
      select.id='background-'+depth;select.dataset.depthChoice=depth;
      label.htmlFor=select.id;label.append(document.createTextNode(layer.name));
      for(const [key,name] of [['','留白'],...W.options(state.scene,depth)]){
        const option=document.createElement('option');option.value=key;option.textContent=name;select.append(option);
      }
      select.value=W.options(state.scene,depth).find(([key])=>state.elements[state.scene].includes(key))?.[0]||'';
      label.append(select);choices.append(label);
      select.addEventListener('change',()=>{
        const keys=W.options(state.scene,depth).map(([key])=>key);
        state.elements[state.scene]=state.elements[state.scene].filter(key=>!keys.includes(key));
        if(select.value)state.elements[state.scene].push(select.value);
        world.build(state);render();
      });
    }
    lastMinute=-1;
  }
  function applyAppearance(){
    const body=M.species[state.body],leg=M.species[state.legs],head=M.species[state.head],shape=A.bodies[state.body],legStyle=A.legs[state.legs];
    ...
    parts['body-markings'].innerHTML=shape.detail;
    const mount=A.headMount(state.head,state.body);
    transform('head-rig',`translate(${mount.x} ${mount.y}) scale(${mount.scale})`);
    transform('backpack',`${translate(shape.bag)} scale(.85)`);
    show('clothes-layer',state.clothes!=='none');
    const details={
      none:'',
      stripe:'<path d="M-90-65H80M-90-43H80M-90-21H80M-90 1H80" stroke="#f0e8cf" stroke-width="10"/>',
      hoodie:'<path d="M-13-87Q6-43 44-76M7-72 5-44m15-29 3 30M-40-30 15-32 29-8-52-8Z" stroke="#eee0c3" stroke-width="4" fill="none"/>',
      overalls:'<path d="M-75-42H60V20H-75Z" fill="#668993"/><path d="M-29-90-19-41M29-90 24-39" stroke="#668993" stroke-width="13"/><path d="M-19-36H18V-14H-19Z" fill="none" stroke="#e0d7b7" stroke-width="2"/><circle cx="-17" cy="-44" r="4" fill="#e3c879"/><circle cx="24" cy="-44" r="4" fill="#e3c879"/>',
      tracksuit:'<path d="M9-89 1 5M-67-75-39-1M-53-82-25 2" fill="none" stroke="#eee5cd" stroke-width="5"/><path d="M23-58h20m-21 8h16" stroke="#eee5cd" stroke-width="3"/>',
    ...
    for(const [i,limb] of limbs.entries()){
      attrs(limb.leg.upper,{fill:legStyle.upperColor});attrs(limb.leg.lower,{fill:legStyle.lowerColor});attrs(limb.leg.ankle,{fill:legStyle.lowerColor});
      attrs(limb.leg.detail,{fill:'none',stroke:legStyle.detail,'stroke-width':legStyle.pattern==='stripes'?4:2});
      attrs(limb.leg.use,{href:'#'+(state.shoes==='natural'?'foot-'+state.legs:'shoe-'+state.shoes),color:leg.leg});
      attrs(limb.arm.outline,{stroke:'#294e46','stroke-width':shape.arm+5});attrs(limb.arm.fill,{stroke:state.body==='panda'?'#50655c':body.color,'stroke-width':shape.arm});
      attrs(limb.arm.sleeve,{stroke:state.color,'stroke-width':shape.arm+1});attrs(limb.arm.hand,{fill:state.body==='panda'?'#50655c':state.body==='fox'?'#735640':body.color,rx:state.body==='pelican'?17:12,ry:state.body==='pelican'?8:10});
      limb.arm.sleeve.style.display=['tee','sweater','raincoat','stripe','hoodie','tracksuit'].includes(state.clothes)?'':'none';
    ...
    $('scene-title').textContent=`${head.name}头、${body.name}身体、${leg.name}腿的旅伴在${W.scenes[state.scene].name}${activityNames[state.activity]}`;
    $('scene-desc').textContent=`可自由混搭角色与穿搭，切换运动、时间、天气和四季。当前是${W.scenes[state.scene].name}，${seasons[state.season].name}${weatherNames[state.weather]}。`;
  }
  function syncMusic(){
    $('music-toggle').setAttribute('aria-pressed',String(music.enabled));$('music-toggle').querySelector('span').textContent=music.enabled?'关闭音乐':'开启音乐';
    $('music-note').textContent=CoastMusic.scores[state.scene].name+' · '+weatherNames[state.weather]+'版本 · '+(music.enabled?'正在播放':'点击开启音乐');
  }
  function syncControls(){
    ...
    for(const key of ['activity','scene','season','weather','color'])document.querySelectorAll(`[data-${key}]`).forEach(el=>el.setAttribute('aria-pressed',String(state[key]===el.dataset[key])));
    ...
    const sound=state.activity==='cycling'?'叮铃铃':state.activity==='running'?'打个节拍':['forest','desert'].includes(state.scene)?'沙沙声':'水花声';$('bell-label').textContent=sound;$('bell').setAttribute('aria-label',state.activity==='cycling'?'按响车铃':sound);
  ...
  function drawLight(){
    ...
    $('scene').style.setProperty('--forest-leaf',mix(season.leaf,'#567269',night*.4));
    $('scene').style.setProperty('--forest-ground',mix(state.season==='winter'?'#d5dfce':season.front,'#567368',night*.5));
    $('scene').style.setProperty('--trail-color',mix(season.ground,'#627a71',night*.4));
    $('scene').style.setProperty('--sand-color',mix(state.season==='winter'?'#d8c9aa':'#d6b47e','#7d8877',night*.45));
    $('scene').style.setProperty('--window-light',night>.45?'#f2ce83':'#dce7d3');
  ...
  function drawWeather(){
    ...
    transform('clouds',`translate(${-t*8%1200} 0)`);
  ...
  function drawActor(){
    const t=clock.motion,pose=M.pose(state.activity,t,A.legs[state.legs]),body=A.bodies[state.body];
    transform('torso-rig',`${translate(pose.hip)} rotate(${pose.tilt})`);
    transform('tail-rig',`${translate(body.tail)} rotate(${Math.sin(t*3)*body.wag})`);
    for(const [i,limb] of limbs.entries()){
      const leg=pose.legs[i],arm=pose.arms[i];
      const drawing=A.legDrawing(leg,state.legs);
      for(const part of ['upper','lower','ankle','detail'])limb.leg[part].setAttribute('d',drawing[part]);
      ...
      if(['tee','stripe'].includes(state.clothes))limb.arm.sleeve.setAttribute('d',`M${arm.start}L${(arm.start[0]+arm.elbow[0])/2} ${(arm.start[1]+arm.elbow[1])/2}`);
  ...
  for(const key of ['activity','scene','season','weather'])document.querySelectorAll(`[data-${key}]`).forEach(el=>el.addEventListener('click',()=>{state[key]=el.dataset[key];if(key==='activity')clock.motion=0;refresh(true);}));
  ...
  $('shuffle-world').addEventListener('click',()=>{
    const next=W.randomEnvironment();clock.hour=next.hour;delete next.hour;Object.assign(state,next);state.autoDay=false;refresh(true);
    $('announcement').textContent='环境已更换：'+W.scenes[state.scene].name+'，'+$('scene-conditions').textContent+'，'+M.formatTime(clock.hour);
  });
  $('music-toggle').addEventListener('click',async()=>{
    try{if(music.enabled)music.stop();else await music.start();syncMusic();}
    catch(error){console.warn('Background music could not start:',error);music.stop();syncMusic();$('music-note').textContent='音乐暂时无法播放，请再次点击开启。';}
  });
  $('music-volume').addEventListener('input',event=>{const value=Number(event.target.value);music.setVolume(value/100);$('volume-value').value=value+'%';});
  $('reset').addEventListener('click',()=>{state=initialState();music.stop();music.setVolume(.4);$('music-volume').value=40;$('volume-value').value='40%';Object.assign(clock,{motion:0,weather:0,hour:10.5});chooseTab('character');refresh(true);$('announcement').textContent='已回到最初的夏日鹈鹕骑行。';});
  $('bell').addEventListener('click',async()=>{
    bellUntil=performance.now()+1000;wake();$('announcement').textContent=state.activity==='cycling'?'叮铃铃，一路好心情。':state.activity==='running'?'嗒、嗒，跟上自己的节奏。':['forest','desert'].includes(state.scene)?'沙沙，草叶与沙粒从身边经过。':'哗啦，把烦恼留在浪里。';
    try{
      audio=getAudio();await audio.resume();
  ...
  document.addEventListener('visibilitychange',()=>{music.setHidden(document.hidden).catch(()=>{music.stop();syncMusic();});if(document.hidden){cancelAnimationFrame(frame);frame=0;previous=0;}else wake();});
