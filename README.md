...
栏目也收录互动动画，例如 `/games/pelican-bicycle/` 的《鹈鹕骑自行车》。这类作品可以省略 `game_rooms` 和未记录的 `game_model` / `game_reasoning`，使用可选的 `play_label`、`cover_caption` 指定入口文案和封面说明；已有游戏保留默认展示。

作品设置 `direct_play: true` 时，栏目标题直接打开 `play_url`，制作记录保留单独按钮。《鹈鹕骑自行车》的独立 HTML 保持原始设计，不注入站点主题或导航。

《鹈鹕骑自行车》v3.2 支持十种生物的头 / 身 / 腿混搭、独立穿搭、自行车 / 跑步 / 划艇，以及海岸 / 森林 / 沙漠 / 城市四场景、四季与五种天气。角色按身体对齐颈部，紧凑躯干搭配各自的尾巴，腿型具有独立比例、粗细、跗部与脚掌轮廓。顶部可分别随机旅伴和环境，各场景五项背景元素按远 / 中 / 近景分组，每层随机选一个，也可手动选择或留白。固定大背景之外，三层分别按路面速度的 10% / 36% / 72% 推进，建筑与植被落在对应地面。划艇分别前进于海面、草丛、沙丘和城市泳池。基础运动与时间速度为 v2 的两倍，1× 下 60 秒循环一天；手动拨动时间会暂停自动循环。

`motion.js` 提供运动和时间纯函数，`anatomy.js` 定义头颈连接、躯干、尾巴和腿型，`world.js` 绘制场景与地表运动，`music.js` 合成四段原创、随天气变化的音乐，`studio.js` 负责 SVG 角色与控件，`studio.css` 保留独立视觉风格。音乐需要点击开启，支持音量与后台暂停；离线使用时保存整个游戏目录。
